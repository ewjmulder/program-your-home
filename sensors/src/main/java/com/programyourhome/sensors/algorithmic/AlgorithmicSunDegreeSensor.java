...
public class AlgorithmicSunDegreeSensor implements SunDegreeSensor {
    ...
    // private static boolean isWithinMargin(final LocalTime localTime1, final LocalTime localTime2) {
    // LocalTime maxCollections.max(Arrays.asList(localTime1, localTime2));
    // }

    // TODO: known limitation: today no such event, but maybe yesterday / tomorrow there is (fix?)
    private boolean isEventWithinMargin(final Function<DayShift, LocalTime> eventFunction, final int margin) {
        ...
        if (timeToday == null) {
            ...
            final LocalTime now = LocalTime.now().truncatedTo(ChronoUnit.MINUTES);
            ...
            if (!withinMargin) {
                ...
                if (minutesNow < margin) {
                    ...
                    withinMargin = timeYesterday != null && timeYesterday.isAfter(now.minusMinutes(margin));
                } else if (this.countMinutesDifference(timeToday, LocalTime.of(23, 59)) < margin) {
                    final LocalTime timeTomorrow = eventFunction.apply(DayShift.YESTERDAY);
                    withinMargin = timeTomorrow != null && timeTomorrow.isBefore(now.plusMinutes(margin));
                }
            }
        }
        return withinMargin;
