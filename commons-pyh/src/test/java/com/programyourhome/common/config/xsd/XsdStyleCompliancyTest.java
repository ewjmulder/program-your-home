package com.programyourhome.common.config.xsd;
 ...
 */
public abstract class XsdStyleCompliancyTest {
    ...
    public void beforeClass() throws Exception {
        ...
        this.document = documentBuilder.parse(XsdStyleCompliancyTest.class.getResourceAsStream(this.getXsdLocation()));
        final XPathFactory xPathfactory = XPathFactory.newInstance();
        this.xpath = xPathfactory.newXPath();
    }

    protected abstract String getXsdLocation();
    ...
    private void assertNoMinMaxOnSequenceChoice() throws Exception {
        for (final Element element : this.getXPathElements("/schema//sequence | /schema//choice")) {
            Assert.assertFalse(this.errorMessage("Do not use the attribute 'minOccurs' on <sequence> and <choice>", element), element.hasAttribute("minOccurs"));
            Assert.assertFalse(this.errorMessage("Do not use the attribute 'maxOccurs' on <sequence> and <choice>", element), element.hasAttribute("maxOccurs"));
        }
    }
    ...
    private void assertAttributeOrder() throws Exception {
        ...
        saxParser.parse(XsdStyleCompliancyTest.class.getResourceAsStream(this.getXsdLocation()), handler);
    }
