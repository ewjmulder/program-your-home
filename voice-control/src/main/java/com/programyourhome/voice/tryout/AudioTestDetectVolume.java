	...
	//	        byte min = min(buffer);
			...
			if (b >  threshold || b < -1 * threshold) {
				...
				if (thresholdBreakTime > lastPeakStart + minTimeBetweenPeaksInMillis) {
	      				// TODO: A peak implies that there was a 'non-peak' (dip) in between.
					// Now it'll work if there is just a lot of noise for a certain time, but we must detect signals lower than
					// the threshold in between: no wait, we must detect that for a certain time there is no threshold break (dip detection :)
